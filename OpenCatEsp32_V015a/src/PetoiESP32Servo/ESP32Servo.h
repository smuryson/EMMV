...
/*
 Added a ServoModel class to customize more parameters
 angleRange, frequency, minPulse, maxPulse;
 
 Revision by Jason Wang and Rongzhong Li.
 Jan 11, 2022
 */
...
#define MIN_PULSE_WIDTH       500     // the shortest pulse sent to a servo, unit is microseconds(us) 
#define MAX_PULSE_WIDTH      2400     // todo the longest pulse sent to a servo, unit is microseconds(us)
#define DEFAULT_PULSE_WIDTH  1500     // default pulse width when servo is attached, unit is microseconds(us)
#define DEFAULT_REFRESH_CPS    50     //default frequency,unit is Hz
#define REFRESH_USEC        20000     //default period, unit is microseconds(us)

/* DEFAULT_PULSE_WIDTH_TICKS = DEFAULT_PULSE_WIDTH/(pulse_period/2**timer_width)
 * default pulse_period is 20000(us), timer_width is 16 (bits)
 * DEFAULT_PULSE_WIDTH_TICKS should be 4915.2, here 4825 is a rough value
 */
#define DEFAULT_PULSE_WIDTH_TICKS 4825

#define DEFAULT_ANGLE_RANGE         180    //default angle range, unit is degree
...
public:
    ...
    int getAngleRange(){return angleRange;}
    int getFrequency(){return frequency;}
    int getMinPulse(){return minPulse;}
    int getMaxPulse(){return maxPulse;};
private:
    int angleRange;
    int frequency;
    int minPulse;
...
public:
    ...
    int attach(int pin, ServoModel model);
    int attach(int pin, ServoModel *model);
	int attach(int pin, int range, int frequency, int min, int max); // as above but also sets min and max values for writes.
	void detach();
...
//   int servoChannel = 0;                              // channel number for this servo
    int angleRange = DEFAULT_ANGLE_RANGE;
	int frequency = DEFAULT_REFRESH_CPS;
