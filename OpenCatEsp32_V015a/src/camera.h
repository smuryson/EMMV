...
#include <MuVisionSensor.h> //you need to download the library https://github.com/mu-opensource/MuVisionSensor3 
//into your ~/Documents/Arduino/libraries/
...
void read_camera() {
  ...
  if (objectDetected[0] || objectDetected[1]) {
    //    if (!(counter % skip)) {
    xDiff = max(min((xCoord - imgRange / 2) / 3, 16), -16);
    yDiff = max(min((yCoord - imgRange / 2) / 3, 12), -12);
