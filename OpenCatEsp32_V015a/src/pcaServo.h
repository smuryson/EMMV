...
class Petoi_PWMServoDriver: public Adafruit_PWMServoDriver {
  private:
    const static int SERVOMIN = 100; //150; // This is the 'minimum' pulse length count (out of 4096)
    const static int SERVOMAX = 520; //600; // This is the 'maximum' pulse length count (out of 4096)
    const static int USMIN = 500; // 600 This is the rounded 'minimum' microsecond length based on the minimum pulse of 150
    ...
    const static long OSCILLATOR_FREQ = 25000000; //25000000
  public:
    int servoPWMFreq;
    long oscillatorFreq;
    float k_us2pulse;
    ...
    void writeAngle(uint8_t servoNum, float angle) {
      //int clipped = min(max(int(k_angle2pulse[PWM_pin[servoNum]] * angle + b_offset), 500), 2400); //todo, should not be fixed here
      //map(value+b_offset, fromLow, fromHigh, toLow, toHigh)
      int clipped = map(angle, 0, ANGLE_RANGE, SERVOMIN, SERVOMAX);
      setPWM(PWM_pin[servoNum], 0, clipped);//clipped); 310 is 90deg, 100 0deg, 520 180deg
      //      if (servoNum == 8 || servoNum == 9 || servoNum == 10 || servoNum == 11) {
      //      if (servoNum == 12 || servoNum == 13 || servoNum == 14 || servoNum == 15) {
      /*                    Serial.print(servoNum); Serial.print('\t'); //todo uncomment
                          //Serial.print(k_angle2pulse[PWM_pin[servoNum]]*angle + b_offset); Serial.print('\t');
                          Serial.print(clipped); Serial.println(" pwm");
