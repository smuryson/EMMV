...
bool lowBattery() {
  ...
  if (currentTime > uptime) {
    ...
    PT(">Current power: "); PT(voltage / vFactor); PT("V: "); PT(voltage); PTL(" raw");
    ...
    if (voltage == 0 || voltage < low_voltage && abs(voltage - lastVoltage) > 10) { //if battery voltage < threshold, it needs to be recharged
      ...
      //      PT(">>> Low power: ");
      //      PT(voltage / vFactor); // PT(voltage * 6.6 / 2520);
      //      PTL("V");
      digitalWrite(LOW_VOLTAGE_LED, HIGH); // Blue LED on, arlarming of low voltage
      ...
      digitalWrite(LOW_VOLTAGE_LED, LOW); // Blue LED off
    }
    if (safeRest) {
      strcpy(lastCmd, "rest");
      loadBySkillName(lastCmd);
...
void resetCmd() { //todo comment print
...
void reaction() {
  if (newCmdIdx) {
    ...
    switch (token) {
      ...
      case T_RAMP: {
          if (token == T_GYRO) {
            checkGyro = false; //todo !checkGyro;
      ...
      case T_BEEP: //beep(tone, duration): tone 0 is pause, duration range is 0~255
        {
          ...
          do {  //it supports combining multiple commands at one time
            ...
            if (token == T_CALIBRATE) {
              ...
              meow(esp_random() % 2 + 1, (esp_random() % 4 + 2) * 10);
            }
